...
class pid_node:
    ...
    def quat_to_angle_Gazebo(self,pose):
        #Returns angle of pose wrt z axis in [0,2pi)
        angle =  2 * np.arccos(pose.orientation.w*np.sign(pose.orientation.z))
        if angle > 3.14159265:
        	angle = angle - 2*np.pi
        print("Angle = ",angle)
        return angle
    ...
    def current_pose_callback(self,data):
        ...
        #print('Current x: ',self.current_pose.pose[1].position)
    ...
    def main(self):
		...
		if self.orient_flag_initial:
		    ...
		    #########################
		    current_angle = self.quat_to_angle_Gazebo(self.current_pose.pose[1])
